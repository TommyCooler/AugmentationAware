...
def prepare_phase2_data(
    ...
    elif dataset_name == "smap_msl_":
        data_path = os.path.join(data_path_base, dataset_name, "processed")
    else:
    ...
    print(
        f"  Test window labels: {test_window_labels.shape} (normal: {np.sum(test_window_labels==0)}, anomaly: {np.sum(test_window_labels==1)})"
    )
    print(
        f"  Test labels: {labels.shape} (normal: {np.sum(labels==0)}, anomaly: {np.sum(labels==1)})"
    )
...
if __name__ == "__main__":
    ...
    # Test with UCR 135
    train_windows, train_labels, test_windows, test_labels, labels = (
        prepare_phase2_data(
            dataset_name="ucr",
            subset="135",
    ...
    train_loader, test_loader = create_phase2_dataloaders(
        train_windows, train_labels, test_windows, test_labels, batch_size=32
    )
